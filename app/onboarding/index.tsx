...
// Step 1: Interests
const INTERESTS = [
    { id: 'weightlifting', name: 'Weightlifting', icon: 'barbell-outline' },
...
export default function OnboardingScreen() {
    ...
    // State
    const [selectedInterests, setSelectedInterests] = useState<string[]>(['desk']);
    ...
    const renderHeader = () => (
        <View className="flex-row justify-between items-center px-6 py-2 pb-4 bg-white z-10">
            {step > 1 ? (
            ...
            {step < 5 && (
                <View className="flex-row items-center space-x-1">
                    ...
                    {step === 3 && <Text className="ml-2 text-gray-400 text-xs text-[10px] font-bold">Step 3/4</Text>}
                </View>
            )}
            {step === 5 && (
                <View className="flex-row items-center space-x-1">
    ...
    const renderStep1 = () => (
        <ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
            ...
            <Text className="text-gray-500 mb-8">Pick a few. We'll suggest tribes you'll enjoy.</Text>
                ...
                {INTERESTS.map((item) => {
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            className={`w-[48%] aspect-square rounded-3xl mb-4 items-center justify-center border-2 relative ${isSelected ? 'bg-green-50 border-green-500' : 'bg-gray-50 border-transparent'}`}
                        >
                            {isSelected && (
                                <View className="absolute top-3 right-3 bg-green-500 w-5 h-5 rounded-full items-center justify-center">
                                    <Ionicons name="checkmark" size={12} color="white" />
                                </View>
                            )}
                            <View className={`w-12 h-12 rounded-full items-center justify-center mb-3 ${isSelected ? 'bg-green-100' : 'bg-white'}`}>
                                <Ionicons name={item.icon as any} size={24} color={isSelected ? '#10B981' : '#4A5568'} />
                            </View>
                            <Text className={`font-bold ${isSelected ? 'text-green-700' : 'text-gray-500'}`}>{item.name}</Text>
                        </TouchableOpacity>
                    );
                })}
            </View>
            ...
            <TouchableOpacity
                onPress={handleNext}
                className="bg-green-500 py-4 rounded-full items-center mb-8"
            >
        ...
        <ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
            ...
            <View className="flex-row justify-between mb-8">
                <TouchableOpacity
                    ...
                    className={`w-[48%] py-8 rounded-2xl border-2 items-center ${isDeskJob ? 'bg-green-50 border-green-500' : 'bg-white border-gray-200'}`}
                >
            ...
            <Text className="text-gray-400 text-xs mb-12 ml-1">We use this to find local groups.</Text>

            <TouchableOpacity
                onPress={handleNext}
                className="bg-green-500 py-4 rounded-full items-center mb-8 relative"
            >
    ...
    // Step 4: Success
    const renderStep4 = () => (
        <ScrollView className="flex-1 px-6" contentContainerStyle={{ flexGrow: 1, paddingBottom: 40, justifyContent: 'center' }} showsVerticalScrollIndicator={false}>
            ...
            {/* Card Preview */}
            <View className="w-full aspect-video bg-gray-900 rounded-2xl overflow-hidden relative mb-8">
            ...
            <TouchableOpacity
                onPress={handleNext}
                className="w-full bg-green-500 py-4 rounded-full items-center mb-4"
            >
                <Text className="text-white font-bold text-lg">Go to My Tribe →</Text>
            </TouchableOpacity>
            <TouchableOpacity className="items-center"><Text className="text-gray-500 font-bold text-sm">Explore more tribes</Text></TouchableOpacity>
        </ScrollView>
    );
    ...
    const renderStep5 = () => (
        <ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
            ...
            <View className="bg-white border border-gray-100 rounded-2xl p-4 mb-8 flex-row justify-between items-center shadow-sm">
                <View className="flex-row items-center flex-1">
                    <View className="w-10 h-10 rounded-full bg-green-50 items-center justify-center mr-3">
